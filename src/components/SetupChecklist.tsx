...
interface SetupChecklistProps {
  trainer: Trainer;
  packages: Package[];
  /** If provided, clicking an item fires this callback instead of navigating via Link.
   *  Useful when the checklist is rendered inside the settings page itself. */
...
export default function SetupChecklist({ trainer, packages, onSelect }: SetupChecklistProps) {
  ...
  return (
    <div className="rounded-2xl border border-[#e5e5ea] p-5 mb-6">
      ...
      <div className="space-y-1">
        {checks.map((check) => {
          const itemClass = "flex items-center gap-3 py-2 px-2 -mx-2 rounded-lg hover:bg-[#f5f5f7] transition-colors group w-full text-left";
          const content = (
            <>
              {/* Checkbox */}
              <div
          ...
          if (onSelect) {
            return (
              <button key={check.id} onClick={() => onSelect(check.tab)} className={itemClass}>
                {content}
              </button>
            );
          }
          return (
            <Link key={check.id} href={`/settings?tab=${check.tab}`} className={itemClass}>
              {content}
            </Link>
          );
        })}
