...
import { Trainer, FormData, TimelineResult, GoalType } from '@/types';
...
function buildWhatsAppMessage(trainer: Trainer, formData: FormData, result: TimelineResult, goalLabel: string): string {
  ...
  if (formData.currentWeight) {
    message += `- Current weight: ${Math.round(formData.currentWeight)}kg\n`;
  }
  if (formData.goalWeight) {
    message += `- Goal weight: ${Math.round(formData.goalWeight)}kg\n`;
  }
  if (formData.age) {
...
function getWhatsAppUrl(trainer: Trainer, formData: FormData, result: TimelineResult, goalLabel: string): string {
...
export default function CTASection({ trainer, formData, result, goalLabel }: CTASectionProps) {
  const whatsappUrl = getWhatsAppUrl(trainer, formData, result, goalLabel);
  ...
  return (
    ...
    >
      ...
      <p className="text-gray-400 text-sm mb-6">
        Tap below to message {trainer.name} directly on WhatsApp with your details
      </p>
      ...
      <a
        ...
        className="inline-flex items-center justify-center gap-3 w-full py-4 rounded-xl text-white font-semibold text-lg transition-all duration-300 hover:scale-[1.02] hover:shadow-lg active:scale-95"
        style={{
      ...
      >
        ...
        Connect with {trainer.name} on WhatsApp
      </a>
      ...
      {trainer.booking_link && (
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="block mt-4 text-gray-400 text-sm hover:text-gray-300 transition-colors underline underline-offset-2"
        >
