    ...
    <div className="min-h-screen bg-gray-950 text-white">
      <div className="max-w-4xl mx-auto px-5 py-10">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold">PT Admin</h1>
            <p className="text-gray-400 text-sm mt-1">Manage trainers and packages</p>
          </div>
          <button
            onClick={() => setShowNew(!showNew)}
            className="bg-orange-500 hover:bg-orange-600 text-white px-5 py-2.5 rounded-xl font-medium transition-colors"
          >
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {trainers.map((trainer) => (
              <div key={trainer.id} className="bg-white/5 rounded-2xl p-6 border border-white/10">
                    ...
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex gap-2">
                        <a
                          href={`/${trainer.slug}`}
                          target="_blank"
                          className="text-gray-400 text-sm hover:text-white transition-colors px-3 py-1.5 rounded-lg bg-white/5"
                        >
                          Preview
                        </a>
                        <button
                    ...
                    {trainer.packages && trainer.packages.length > 0 && (
                      <div className="mt-4 flex gap-2 flex-wrap">
                        {trainer.packages.map((pkg) => (
                          <span key={pkg.id} className="text-xs bg-white/5 px-3 py-1 rounded-full text-gray-400">
                            {pkg.name} — {pkg.is_online ? 'Online' : `${pkg.sessions_per_week}x/wk`}
                            {pkg.monthly_price ? ` — £${pkg.monthly_price}/mo` : ''}
                          </span>
                        ))}
                      </div>
                    )}
...
function TrainerForm({
  ...
  const inputClass = "w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white text-sm placeholder-gray-500 focus:outline-none focus:border-white/30";
  ...
  return (
    <div className="space-y-6 bg-white/[0.02] rounded-xl p-5 border border-white/5">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          <label className="text-gray-400 text-xs block mb-1">Slug * (URL path)</label>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={addPackage} className="text-xs text-orange-400 hover:text-orange-300">+ Add package</button>
        </div>
        <div className="space-y-3">
          {pkgs.map((pkg, i) => (
            <div key={i} className="grid grid-cols-6 gap-2 items-end bg-white/[0.02] p-3 rounded-lg">
              <div className="col-span-2">
                <label className="text-gray-500 text-[10px] block mb-0.5">Name</label>
                <input value={pkg.name} onChange={(e) => updatePackage(i, 'name', e.target.value)} placeholder="2x Per Week" className={inputClass} />
              </div>
              <div>
              ...
              <div className="flex items-center gap-2">
                ...
                {pkgs.length > 1 && (
                  <button onClick={() => removePackage(i)} className="text-red-500 text-xs hover:text-red-400">Remove</button>
                )}
              </div>
            </div>
          ))}
      ...
      <div className="flex gap-3 justify-end">
        <button onClick={onCancel} className="px-5 py-2 rounded-lg text-gray-400 text-sm hover:text-white transition-colors">Cancel</button>
        <button onClick={handleSave} disabled={saving || !form.name || !form.slug || !form.booking_link} className="bg-orange-500 hover:bg-orange-600 disabled:opacity-40 text-white px-6 py-2 rounded-lg text-sm font-medium transition-colors">
          {saving ? 'Saving...' : 'Save'}
        </button>
      </div>
    </div>
